...
public class CursoDeleteCommand
{
    ...
    internal class CursoDeleteCommandHandler : IRequestHandler<CursoDeleteCommandRequest, Result<Unit>>
    {
        ...
        {
            var curso = await _context.Cursos!.Include(x => x.Instructores).Include(x => x.Precios).Include(x => x.Calificaciones).Include(x => x.Photos).FirstOrDefaultAsync(x => x.Id == request.CursoId);
            ...
            return resultado ? Result<Unit>.Success(Unit.Value) : Result<Unit>.Failure("Error al eliminar el curso");
        }
    }
    ...
    public class CursoDeleteCommandRequestValidator : AbstractValidator<CursoDeleteCommandRequest>
    {
        ...
        {
            RuleFor(x => x.CursoId).NotNull().WithMessage("El cursoId es requerido");
        }
    }
}